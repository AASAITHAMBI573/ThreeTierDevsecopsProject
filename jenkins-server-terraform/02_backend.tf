...
terraform {
  backend "s3" {
    ...
    bucket         = "three-tier-devsecops-project-05"
    ...
    key            = "three-tier-devsecops-project-05/jenkins-server-terraform/terraform.tfstate"
    
    # The DynamoDB table used for state locking to prevent concurrent state changes.
    dynamodb_table = "three-tire-lock-files"
